...
- WebUI 이동: 우측 JumpBar/list 클릭이 현재 정렬 기준에서도 실제 항목으로 스크롤되도록 보정했습니다.
...
- GHCR `9.0.10-2`와 `latest`가 같은 multi-arch 이미지로 발행됨
- 운영 컨테이너를 `9.0.10-2`로 교체했고 `/api/health=Ok`, Docker health `healthy`를 확인함
