...
이번 버전은 OPDS 다음 권·완독·수정일 정렬, EPUB 목차·표지·본문 경로 오류, ZIP/CBZ 페이지 누락과 다운로드 파일명·실패 처리를 수정합니다. 검증 범위와 외부 앱 회전 제한은 [릴리스 노트](RELEASE_NOTES.md)를 확인하세요.
...
- 선택적 EPUB 목차·표지 오류를 보정하고 정상 본문을 엽니다. 중복 spine 읽기 순서, 중첩 장 경계와 리소스 경로도 처리합니다.
- 중복 ZIP 항목과 여러 압축파일·낱장 이미지가 섞인 챕터의 모든 페이지를 보존합니다. 실패한 추출은 정상 캐시로 재사용하지 않습니다.
- 다운로드를 요청별로 작성하고 메타데이터 기반 이름을 사용합니다. 실패·취소 시 임시 파일을 정리하며 ZIP 작성 완료 후 완료 상태를 표시합니다.
- OPDS 다음 권 중복 추천과 마지막 권 이후 되돌아감을 수정하고, 라이브러리 피드는 마지막 콘텐츠 수정일순으로 제공합니다. OPDSy처럼 제목순으로 재정렬하는 앱에서는 서버 정렬이 화면에 반영되지 않을 수 있습니다.
- 기존 진행률 보호·90% 다음 권 추천과 사용자별 프로필을 유지합니다. 기존 진행률을 일괄 변경하지 않습니다.

플랫폼별 검증과 기존 외부 앱 제한은 [RELEASE_NOTES.md](RELEASE_NOTES.md)를 참고하세요.
