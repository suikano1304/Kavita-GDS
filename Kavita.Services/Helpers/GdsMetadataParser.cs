...
public static class GdsMetadataParser
{
    ...
    private static ComicInfo BuildComicInfo(string filePath, ComicInfo? baseInfo, Func<string, string?> metadata)
    {
        ...
        Apply(metadata, "Summary", value => info.Summary = NormalizeSummaryLineBreaks(value));
    ...
    // Older GDS sidecars store line breaks as literal escape sequences, including
    // single-quoted/plain YAML scalars. Decode only line breaks, never arbitrary escapes.
    internal static string NormalizeSummaryLineBreaks(string value)
    {
    ...
    private static bool TryNormalizeBase64Cover(string value, out string encodedImage)
    {
        ...
        if (string.IsNullOrWhiteSpace(encodedImage) || !System.Buffers.Text.Base64.IsValid(encodedImage))
        {
            encodedImage = string.Empty;
            return false;
        }
    ...
    private static LargeYamlInfo ReadLargeYamlInfo(string yamlPath)
    {
        ...
        // Parse only the small metadata mapping with YAML semantics. Never feed the
        // potentially multi-megabyte file-cover mapping to the YAML object model.
        const int maxMetadataChars = 256 * 1024;
        var metadataYaml = new StringBuilder();
        var metadataOverflow = false;
        var inMeta = false;
        var inFiles = false;
        ...
        foreach (var cappedLine in ReadCappedLines(yamlPath, MaxLargeYamlLinePrefixChars))
        {
            ...
            if (inMeta && (string.IsNullOrWhiteSpace(line) || char.IsWhiteSpace(line[0])))
            {
                if (cappedLine.IsTruncated || metadataYaml.Length + line.Length + 1 > maxMetadataChars)
                    metadataOverflow = true;
                if (!metadataOverflow) metadataYaml.AppendLine(line);
            }
        ...
        if (!metadataOverflow && metadataYaml.Length > 0)
        {
            try
            {
                var yaml = new YamlStream();
                yaml.Load(new StringReader(metadataYaml.ToString()));
                if (yaml.Documents.Count == 1 && yaml.Documents[0].RootNode is YamlMappingNode mapping)
                {
                    foreach (var pair in mapping.Children)
                    {
                        if (pair.Key is YamlScalarNode {Value: not null} key &&
                            pair.Value is YamlScalarNode {Value: not null} value)
            ...
            {
                // Keep the line-based fallback for damaged sidecars.
            }
        }
