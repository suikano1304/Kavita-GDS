...
public class GdsMetadataParserTests : IDisposable
{
    ...
    [InlineData("첫 줄\r\n둘째 줄", "첫 줄\n둘째 줄")]
    [InlineData("첫 줄\\n\\n다음 문단", "첫 줄\n\n다음 문단")]
    [InlineData(@"유니코드 \u0041 / 탭 \t", @"유니코드 \u0041 / 탭 \t")]
    public void NormalizeSummaryLineBreaks_PreservesLegacyParagraphs(string input, string expected)
    {
        Assert.Equal(expected, GdsMetadataParser.NormalizeSummaryLineBreaks(input));
        Assert.Equal(expected, GdsMetadataParser.NormalizeSummaryLineBreaks(expected));
    }
    ...
    [InlineData("'첫 줄\\n둘째 줄'")]
    [InlineData("\"첫 줄\\n둘째 줄\"")]
    public void GetComicInfo_DecodesLegacySummaryOnly(string scalar)
    {
    ...
    [InlineData(">-\n        첫 줄\n        둘째 줄", "첫 줄 둘째 줄")]
    [InlineData("|-\n        첫 문단\n\n        다음 문단", "첫 문단\n\n다음 문단")]
    public void GetComicInfo_PreservesYamlMultilineSummary(string scalar, string expected)
    {
        File.WriteAllText(Path.Join(_testDirectory, "kavita.yaml"),
            "meta:\n    Summary: " + scalar + "\n    Writer: Author\nfiles:\n    sample.epub:\n        page: 7\n");
        var info = GdsMetadataParser.GetComicInfo(_bookPath);
        Assert.NotNull(info);
        Assert.Equal(expected, info.Summary);
        Assert.Equal("Author", info.Writer);
        Assert.True(GdsMetadataParser.TryGetPageCount(_bookPath, out var pages));
