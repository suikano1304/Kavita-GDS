...
Kavita official `0.9.0.10` nightly 기반 GDS scanfix 빌드 `9.0.10-2`를 Docker/GHCR 배포용으로 패키징했습니다.
...
- `9.0.10-2`에는 `9.0.7-6`까지의 WebUI, reader/cache, scanner, cover 관련 GDS hotfix와 `9.0.10-1` official base port를 포함했습니다.
...
- GHCR `9.0.10-2`와 `latest`는 같은 amd64/arm64/armv7 multi-arch manifest를 가리킵니다.
