...
- `linux/arm/v7` 이미지는 같은 소스와 prebuilt production UI로 빌드해 GHCR multi-arch manifest에 포함했고, CT101 qemu smoke test에서 pushed GHCR unified tag의 `/api/health` `Ok`를 확인했습니다.
- GHCR `9.0.10-1`와 `latest`는 같은 amd64/arm64/armv7 multi-arch manifest를 가리킵니다.
