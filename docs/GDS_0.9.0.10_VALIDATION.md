...
## 2026-06-23 `9.0.10-3` release

- GHCR `9.0.10-3`와 `latest`는 같은 multi-arch manifest를 가리킨다.
- 포함 플랫폼은 `linux/amd64`, `linux/arm64`, `linux/arm/v7`이다.
- official Kavita `0.9.0.10` nightly base와 기존 GDS patch set을 유지했다.
- WebUI JumpBar가 현재 정렬 기준과 실제 정렬 위치를 사용하도록 보강했다. 최근 수정순처럼 같은 날짜가 큰 block을 이루는 목록에서도 여러 직접 jump target을 제공한다.
- Docker runtime image에 오래된 WebUI asset이 남지 않도록 `wwwroot` 정리 후 production bundle을 복사한다.

```text
multiarch digest=sha256:b3f9ed89796cbdfb24e0dee44bbf7a8d5a4bd72cdd3333f333110cc5836da8dd
...
- GHCR `linux/arm/v7` pushed image reached `/api/health` `Ok` under CT101 qemu.
- Production `kavita` was rolled to the GHCR `9.0.10-3` amd64 image after a controlled SQLite online backup. Local and routed `/api/health` returned `Ok`, Docker health reached `healthy`, DB quick_check returned `ok`, and no SQLite/disk/database-lock startup errors were observed.
