...
Kavita GDS는 Kavita official `0.9.1.4` source에 GDS/rclone scanfix를 포팅한 비공식 Docker 빌드입니다.

현재 릴리즈는 `0.9.1.4-6`입니다. GHCR의 `0.9.1.4-6`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함합니다.
...
```text
ghcr.io/suikano1304/kavita-gds:0.9.1.4-6
```
...
services:
  kavita:
    image: ghcr.io/suikano1304/kavita-gds:0.9.1.4-6
...
`0.9.1.4-6`의 기본 배포물은 GHCR multi-arch image입니다. Docker pull이 불가능한 폐쇄망 환경에서는 GHCR image를 별도 registry로 미러링하거나, 필요한 플랫폼의 OCI archive를 따로 생성해 반입하세요. 과거 릴리스의 GitHub tarball은 해당 릴리스 태그 기준 산출물입니다.
...
`0.9.1.4-6`는 다음 EPUB/WebUI/웹소설 metadata 문제를 완화합니다.
...
python3 scripts/summarize_kavita_scan_logs.py \
  /your/kavita/config/logs/kavitaYYYYMMDD.log
```
...
python3 scripts/analyze_kavita_reader_latency.py \
  /your/kavita/config/logs/kavitaYYYYMMDD.log \
  --db /your/kavita/config/kavita.db \
  --cache-dir /your/kavita/config/cache \
...
`0.9.0.2-5` 이후와 `0.9.1.4-6` 이미지는 production UI bundle을 포함합니다.
...
- `linux/arm/v7`는 Windows Docker Desktop/WSL qemu smoke test에서 `/api/health` `Ok`를 확인했지만, native ARMv7 실서비스 검증은 별도 환경에서 다시 확인하는 것이 좋습니다.
