...
현재 릴리즈는 `0.9.1.4-3`입니다. GHCR의 `0.9.1.4-3`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함합니다.
...
`0.9.1.4-3`의 기본 배포물은 GHCR multi-arch image입니다. Docker pull이 불가능한 폐쇄망 환경에서는 GHCR image를 별도 registry로 미러링하거나, 필요한 플랫폼의 OCI archive를 따로 생성해 반입하세요. 과거 릴리스의 GitHub tarball은 해당 릴리스 태그 기준 산출물입니다.
...
`0.9.1.4-3`는 다음 EPUB/WebUI/웹소설 metadata 문제를 완화합니다.
...
`0.9.0.2-5` 이후와 `0.9.1.4-3` 이미지는 production UI bundle을 포함합니다.
