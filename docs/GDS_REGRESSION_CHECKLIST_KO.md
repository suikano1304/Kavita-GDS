...
- [ ] `scan-folder` API: 상위 FolderPath가 같은 두 작품에서 원래 요청 경로를 보존하고 요청한 작품만 스캔한다. 미일치 시 AbortOnNoSeriesMatch 설정과 기존 폴더 감시 동작을 유지한다.

### OPDS·프로필·폰트 회귀 검증

- [ ] 챕터·읽기 목록 acquisition을 표지보다 먼저 제공하고 실제 응답의 MIME·파일명·페이지 수와 일치시킨다. 합본 생성 전 크기는 미확정으로 표시한다.
- [ ] CBZ는 표준 MIME과 `.zip` acquisition 별칭, `.cbz` 저장 파일명을 제공한다. 기존 URL 및 EPUB/TXT/PDF 다운로드도 유지한다.
- [ ] 여러 원본 압축파일·낱장 이미지의 전체 바이트와 자연정렬을 검증한다. 동시 다운로드와 Range 요청에서도 완성된 동일 파일을 반환하고 임시 파일을 정리한다.
- [ ] Panels 여부와 진행률 저장 허용의 네 조합을 DB로 확인한다. 다른 사용자의 진행률은 바뀌지 않는다.
- [ ] 두 계정·두 기기에서 자동 저장, 연결 프로필, 라이브러리, 기본값 우선순위를 확인한다. 타인 프로필·기기 ID의 조회/변경/삭제/연결 요청은 거부한다.
- [ ] 이미지 맞춤을 명시적으로 바꾼 경우 자동·상위 저장과 재열기에 반영한다. 초기화나 읽기 모드 변경만으로 기존 Automatic을 덮어쓰지 않는다.
- [ ] 로그아웃·계정 전환 시 이전 계정의 화면 상태와 지연 저장 요청을 폐기한다.
- [ ] EPUB의 중첩 요소와 inline `!important`에도 선택 폰트를 적용한다. 기본값은 원래 선언과 우선순위를 복원한다. 본문 밖 스타일은 보존한다.
