...
OPDS 중복·역순 다음 권 추천, 완독 경계와 라이브러리 수정일순 피드를 보정했습니다. EPUB 목차·본문·리소스, 표지 저장과 페이지 수 보정 경쟁, ZIP/CBZ 전체 페이지 보존·캐시 복구·다운로드 이름 및 취소 정리를 수정했습니다. 검증과 앱별 제한은 [릴리스 노트](../RELEASE_NOTES.md)를 확인하세요.

# 0.9.1.4-5

OPDS 자동·동시 이미지 요청의 진행률 후퇴를 막고, 웹과 OPDS의 이어읽기를 가장 뒤의 진행 권과 90% 추천 기준으로 통일했습니다. 저장 페이지와 DB 구조는 보존하며 추천 종료 시 continue-point API는 204를 반환합니다. 검증과 호환성은 [릴리스 노트](../RELEASE_NOTES.md)를 확인하세요.

# 0.9.1.4-4

OPDS 다음 항목 선행 준비, 실행 그룹 I/O 보호와 제한 재시도, 관리자 기존 이메일 저장 호환성과 오류 후 재시도, 읽기 전용 연령 설정 표시를 수정했습니다. 기존 프로필 DTO/DB 구조와 OPDS URL을 유지합니다.

# 0.9.1.4-3

- OPDS acquisition 형식·정렬·합본 및 진행률 저장 조건 수정.
- 읽기 프로필 맞춤 저장·사용자/기기 격리·계정 전환 처리 보강.
- EPUB 중첩 폰트 적용·기본값 복원 및 캐시 삭제 경쟁 상태 재시도.
- 검증 범위와 외부 앱의 남은 회전 제한은 [릴리스 노트](../RELEASE_NOTES.md)에 명시.

# 0.9.1.4-2

- `scan-folder` 요청의 원래 작품 경로를 보존하여 공유 상위 폴더 때문에 전체 라이브러리 스캔으로 확대되는 문제를 수정했습니다.
- API가 작품 조회와 스캔 예약 완료를 기다리도록 보완했습니다.
- 세 아키텍처 기동, 폴더 스캔 API, 기존 DB 및 회귀 검증을 통과했습니다.
